...
const [reactPluginResult, reactHooksPluginResult, jsxA11yPluginResult] = await Promise.all([
  loadOptionalPlugin('eslint-plugin-react'),
  loadOptionalPlugin('eslint-plugin-react-hooks'),
  loadOptionalPlugin('eslint-plugin-jsx-a11y'),
]);

const hasReactPlugins =
  reactPluginResult.available && reactHooksPluginResult.available && jsxA11yPluginResult.available;
  ...
  if (hasReactPlugins) {
    return [
      ...baseConfig,
      {
        ...baseReactConfig,
        plugins: {
          react: reactPluginResult.plugin,
          'react-hooks': reactHooksPluginResult.plugin,
          'jsx-a11y': jsxA11yPluginResult.plugin,
        },
        settings: {
          react: {
            version: 'detect',
