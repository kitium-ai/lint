...
test("eslintNodeConfig should have Node.js-specific globals", () => {
  ...
  assert(
    config.languageOptions?.globals?.__dirname === "readonly",
    "should have __dirname global",
  );
  assert(
    config.languageOptions?.globals?.require === "readonly",
    "should have require global",
  );
...
test("eslintJestConfig should export an object", () => {
  assert(
    typeof eslintJestConfig === "object",
    "jestConfig should be an object",
  );
...
test("eslintJestConfig should have Jest globals", () => {
  ...
  assert(
    eslintJestConfig.languageOptions?.globals?.it === "readonly",
    "should have it global",
  );
  assert(
...
test("eslintGraphQLConfig should export an object", () => {
  assert(
    typeof eslintGraphQLConfig === "object",
    "graphqlConfig should be an object",
  );
...
test("eslintNextjsConfig should export an object", () => {
  assert(
    typeof eslintNextjsConfig === "object",
    "nextjsConfig should be an object",
  );
  assert(
    eslintNextjsConfig.plugins?.["@next/next"],
    "should have @next/next plugin",
  );
});

test("eslintSecurityConfig should export an object", () => {
  assert(
    typeof eslintSecurityConfig === "object",
    "securityConfig should be an object",
  );
...
test("eslintKitiumConfig should have Kitium plugin", () => {
  const config = eslintKitiumConfig.find(
    (c) => c.name === "kitium/component-standards",
  );
